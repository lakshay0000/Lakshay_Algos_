...
class algoLogic(optOverNightAlgoLogic):
    ...
    def _load_schedule(self, path):
        """Load schedule CSV and ensure datetime column is parsed."""
        if not os.path.exists(path):
            self.strategyLogger.info(f"Schedule file not found: {path}")
            return pd.DataFrame()
        df_sched = pd.read_csv(path)
        if 'datetime_ist' in df_sched.columns:
            df_sched['datetime_ist'] = pd.to_datetime(df_sched['datetime_ist'], errors='coerce')
        return df_sched
    
    def _get_scheduled_stocks(self, sched_df, current_dt):
        """
        Return (pos_list, neg_list) for the given current datetime.
        Matching strategy:
         - Try exact datetime match (date+time)
         - Fallback to time-only match (same clock time)
        """
        if sched_df.empty:
            return [], []

        cur = current_dt.replace(second=0, microsecond=0)

        # exact datetime match
        mask = (sched_df['datetime_ist'] == cur)
        if not mask.any():
            # fallback to time-only match
            mask = (sched_df['datetime_ist'].dt.time == cur.time())

        if not mask.any():
            return [], []

        row = sched_df[mask].iloc[0]

        pos_cols = [c for c in sched_df.columns if c.upper().startswith('POS_')]
        neg_cols = [c for c in sched_df.columns if c.upper().startswith('NEG_')]

        pos_list = [row[c] for c in pos_cols if pd.notna(row.get(c))]
        neg_list = [row[c] for c in neg_cols if pd.notna(row.get(c))]

        # convert to list of strings and remove empties
        pos_list = [str(x).strip() for x in pos_list if str(x).strip() and str(x) != 'nan']
        neg_list = [str(x).strip() for x in neg_list if str(x).strip() and str(x) != 'nan']
    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        schedule_path = "/root/Lakshay_Algos/stocksList/NIFTY_IST_StockNames_CLEANED_NoISO_3.csv"
        sched_df = self._load_schedule(schedule_path)
        ...
        for stock in stock_list:
            ...
            try:
                # Fetch historical data for backtesting
                df_1min = getEquityBacktestData(stock, startEpoch, endEpoch, "1Min", conn=conn)
        ...
        for timeData in df.index: 
            ...
            if (self.humanTime.time() < time(9, 17)) | (self.humanTime.time() > time(15, 30)):
                continue


            # if schedule matches this time, override selected_stocks
            if self.humanTime.minute == 17:
                pos_list, neg_list = self._get_scheduled_stocks(sched_df, self.humanTime - timedelta(minutes=2))
                if pos_list or neg_list:
                    # keep order: positives then negatives
                    selected_stocks = pos_list + neg_list
                    self.strategyLogger.info(f"{self.humanTime} Scheduled Stocks - Positives: {pos_list}, Negatives: {neg_list}")
            ...
            for stock in selected_stocks:
                ...
                if not self.openPnl.empty:
                    for index, row in self.openPnl.iterrows():
                        ...
                        if symSide not in selected_stocks:
                            exitType = "OutOfList Exit"
                            self.exitOrder(index, exitType)
                ...
                if ((timeData-60) in df_1min.index):

                    if (stock in neg_list) and (state["stockcount"] ==0):
                            
                            entry_price = df_1min.at[lastIndexTimeData[1], "c"]

                            self.entryOrder(entry_price, stock, (amountPerTrade//entry_price), "SELL")
                            self.strategyLogger.info(f"Entered SELL for {stock} at {entry_price}")

                
                    if (stock in pos_list) and (state["stockcount"] ==0):
                            
                            entry_price = df_1min.at[lastIndexTimeData[1], "c"]

                            self.entryOrder(entry_price, stock, (amountPerTrade//entry_price), "BUY")
                            self.strategyLogger.info(f"Entered BUY for {stock} at {entry_price}")
