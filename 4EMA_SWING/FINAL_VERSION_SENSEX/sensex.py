...
class algoLogic(optOverNightAlgoLogic):
    ...
    # Calculate the 200-period EMA
        ...
        for timeData in df.index: 
            ...
            if ((timeData-900) in df_15min.index):
                if len(maxlist)>=2 and len(lowlist)>=2 and (df_15min.at[last15MinIndexTimeData[1], "EMA_High"] - df_15min.at[last15MinIndexTimeData[1], "EMA_Low"])<300:
                    if len(maxlist)>=2:
                    ...
                    if callCounter==0:
                        list1_low.clear()
                    self.strategyLogger.info(f"{self.humanTime}\tTwoswinghigh: {Twoswinghigh}\tTwoswinglow: {Twoswinglow}\tPUT&CALLEntryAllow: TRUE\tMidhigh: {Midhigh}")
            ...
            if not self.openPnl.empty:
                for index, row in self.openPnl.iterrows():
                    ...
                    if symSide == "PE":
                        if UnderlyingPrice <= (row["IndexPrice"]-300):
