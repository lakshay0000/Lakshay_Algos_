    ...
    # Calculate the 200-period EMA
        ...
        for timeData in df.index: 
            ...
            if not self.openPnl.empty:
                for index, row in self.openPnl.iterrows():
                    ...
                    if row["CurrentPrice"] <= row["Target"]:
                        if row["TradeType"] == "FirstTrade":
                            exitType = "VirtualTrade"
            ...
            if ((timeData-900) in df_15min.index):
                ...
                if putCounter < 3 and callCounter==0:
                ...
                if PutEntryAllow and df_15min.at[last15MinIndexTimeData[1], "%K"] > 80:
                    ...
                    if putCounter == 0:
                        self.entryOrder(data["c"], putSym, lotSize, "SELL", {"Expiry": expiryEpoch, "EmgStoploss": EmgStoploss, "Target":target, "TradeType": "FirstTrade"},)
                    else:
                        self.entryOrder(data["c"], putSym, lotSize, "SELL", {"Expiry": expiryEpoch, "EmgStoploss": EmgStoploss, "Target":target, "TradeType": "OtherTrade"},)
                    PutEntryAllow = False
                    CallEntryAllow = False
                    swinghigh = None
                ...
                if callCounter < 3 and putCounter==0:
                ...
                if CallEntryAllow and df_15min.at[last15MinIndexTimeData[1], "%K"] < 20:
                    ...
                    if callCounter == 0:
                        self.entryOrder(data["c"], callSym, lotSize, "SELL", {"Expiry": expiryEpoch, "EmgStoploss": EmgStoploss, "Target":target, "TradeType": "FirstTrade"},)
                    else:
                        self.entryOrder(data["c"], callSym, lotSize, "SELL", {"Expiry": expiryEpoch, "EmgStoploss": EmgStoploss, "Target":target, "TradeType": "OtherTrade"},)
                    CallEntryAllow = False
                    PutEntryAllow = False
                    swinglow = None
