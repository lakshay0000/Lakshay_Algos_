...
class algoLogic(optOverNightAlgoLogic):
    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        pe_prev_day_high = None
        pe_prev_day_low = None
        ce_prev_day_high = None
        ce_prev_day_low = None
        ...
        for timeData in df.index: 
            ...
            if self.humanTime.time() == time(9, 16):
                ...
                self.strategyLogger.info(f"df_CE:{df_CE} {callSym}")
                df_1d_CE = getFnoBacktestData(callSym, prev_day, prev_day+22500, "1Min")
                self.strategyLogger.info(f"df_1d_CE:{df_1d_CE}")

                df_PE = getFnoBacktestData(putSym, open_epoch, open_epoch + 86400, "1Min")
                self.strategyLogger.info(f"df_PE:{df_PE} {putSym}")
                df_1d_PE = getFnoBacktestData(putSym, prev_day, prev_day+22500, "1Min")
                self.strategyLogger.info(f"df_1d_PE:{df_1d_PE}")
                ...
                for index, row in self.openPnl.iterrows():
                    ...
                    if self.humanTime.time() >= time(15, 20):
                        ...
                        if row["CurrentPrice"] <= row["Target"]:
                            ...
                            self.strategyLogger.info(f"df_CE:{df_CE} {callSym}")
                            df_1d_CE = getFnoBacktestData(callSym, prev_day, prev_day+22500, "1Min")
                            self.strategyLogger.info(f"df_1d_CE:{df_1d_CE} {callSym}")
                            ...
                            if df_CE.at[lastIndexTimeData[1], "c"] > ce_prev_day_high:
                        ...
                        if row["CurrentPrice"] <= row["Target"]:
                            ...
                            self.strategyLogger.info(f"df_PE:{df_PE} {putSym}")
                            df_1d_PE = getFnoBacktestData(putSym, prev_day, prev_day+22500, "1Min")
                            self.strategyLogger.info(f"df_1d_PE:{df_1d_PE} {putSym}")
                            ...
                            if df_PE.at[lastIndexTimeData[1], "c"] > pe_prev_day_high:
            ...
            if ((timeData-60) in df.index) and self.humanTime.time() < time(15, 20):
                ...
                if (lastIndexTimeData[1] in df_CE.index) and callCounter < 1:
                    if df_CE.at[lastIndexTimeData[1], "c"] < ce_prev_day_low:

                        entry_price = df_CE.at[lastIndexTimeData[1], "c"]
                        target = 0.1 * entry_price
                        stoploss = 2 * entry_price
                ...
                if (lastIndexTimeData[1] in df_PE.index) and putCounter < 1:
                    if df_PE.at[lastIndexTimeData[1], "c"] < pe_prev_day_low:
