                    ...
                    if (lastIndexTimeData[1] in df_CE.index) and callCounter < 1:
                        if df_CE.at[lastIndexTimeData[1], "HRSO"] < CE_Low and CE_Target == False:
                            ...
                            try:
                                ...
                                data_OTM1 = self.fetchAndCacheFnoHistData(
                                    callSym_OTM1, lastIndexTimeData[1])
                            ...
                            self.entryOrder(entry_price, callSym, lotSize, "SELL", {"Expiry": expiryEpoch,"Target": target,"stoploss":stoploss, "TradeType": "Main"},)
                            self.entryOrder(data_ITM["c"], callSym_ITM, lotSize, "SELL")
                            self.entryOrder(data_OTM1["c"], callSym_OTM1, lotSize, "SELL")
                    ...
                    if (lastIndexTimeData[1] in df_PE.index) and putCounter < 1:
                        if df_PE.at[lastIndexTimeData[1], "HRSO"] < PE_Low and PE_Target == False:
                            ...
                            try:
                                ...
                                data_OTM1 = self.fetchAndCacheFnoHistData(
                                    putSym_OTM1, lastIndexTimeData[1])
                                data_OTM2 = self.fetchAndCacheFnoHistData(  
                                    putSym_OTM2, lastIndexTimeData[1])
                            except Exception as e:
                                self.strategyLogger.info(e)

                            self.entryOrder(entry_price, putSym, lotSize, "SELL", {"Expiry": expiryEpoch,"Target": target,"stoploss":stoploss, "TradeType": "Main"},)
                            self.entryOrder(data_ITM["c"], putSym_ITM, lotSize, "SELL")
                            self.entryOrder(data_OTM1["c"], putSym_OTM1, lotSize, "SELL")
