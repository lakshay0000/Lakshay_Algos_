        ...
        for timeData in df.index: 
                ...
                for index, row in self.openPnl.iterrows():
                    ...
                    if self.humanTime.time() >= time(15, 20):
                        ...
                        if row["CurrentPrice"] <= row["Target"]:
                            ...
                            df_CE['range'] = df_CE['h'].cummax() - df_CE['l'].cummin()
                            df_CE['HRSO'] = ((df_CE['c'] - df_CE['l'].cummin()) / df_CE['range'])*100
                            self.strategyLogger.info(f"{self.humanTime} df_CE (first 20 rows):\n{df_CE.head(20).to_string()} {callSym}")
                            ...
                            if df_CE.at[lastIndexTimeData[1], "HRSO"] > 70:
                        ...
                        if row["CurrentPrice"] <= row["Target"]:
                            ...
                            df_PE['range'] = df_PE['h'].cummax() - df_PE['l'].cummin()
                            df_PE['HRSO'] = ((df_PE['c'] - df_PE['l'].cummin()) / df_PE['range'])*100
                            self.strategyLogger.info(f"{self.humanTime} df_PE:{df_PE} {putSym}")
            ...
            if ((timeData-60) in df.index) and self.humanTime.time() < time(15, 20) and self.humanTime.time() > time(9, 16):
                ...
                if (lastIndexTimeData[1] in df_CE.index) and callCounter < 1:
                    if df_CE.at[lastIndexTimeData[1], "HRSO"] < 30:
