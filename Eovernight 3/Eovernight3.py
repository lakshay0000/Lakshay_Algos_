...
class algoLogic(baseAlgoLogic):
    ...
    def backtest(self, stockName, startDate, endDate):
        ...
        df_5min['ema9'] = df_5min['c'].ewm(span=9, adjust=False).mean()
        df_5min['ema21'] = df_5min['c'].ewm(span=21, adjust=False).mean()
        df_5min.dropna(inplace=True)

        df_5min["emaUcross"]= np.where((df_5min["ema9"] > df_5min["ema21"]) & (df_5min["ema9"].shift(1) < df_5min["ema21"].shift(1)), 1, 0)
        df_5min["emaDcross"] = np.where((df_5min["ema9"] < df_5min["ema21"]) & (df_5min["ema9"].shift(1) > df_5min["ema21"].shift(1)), 1, 0)
        ...
        for timeData in df.index:
            ...
            if not stockAlgoLogic.openPnl.empty:
                for index, row in stockAlgoLogic.openPnl.iterrows():
                    if row["PositionStatus"] == 1:
                        if (df_5min.at[last5MinIndexTimeData[1], "emaDcross"] == 1):
            ...
            if ((timeData-300) in df_5min.index) & (stockAlgoLogic.openPnl.empty):
                if (df_5min.at[last5MinIndexTimeData[1], "emaUcross"] == 1) and (df_5min.at[last5MinIndexTimeData[1], "Supertrend"] == 1):
