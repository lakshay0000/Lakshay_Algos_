                ...
                if ((timeData-60) in df_1min.index) and (self.humanTime.time() < time(15, 20)):
                    ...
                    if state["main_trade"]:
                        ...
                        if (df_1min.at[lastIndexTimeData[1], "c"] > state["High"]):
                            ...
                            if target < (entry_price+((Mean/100)*entry_price)):
                                self.strategyLogger.info(f"{self.humanTime} -----------SKIPPING BUY TRADE------------")
                                self.strategyLogger.info(f"{self.humanTime} {stock} target: {target}, Min Target not met: {entry_price+((Mean/100)*entry_price)}")
                                continue
