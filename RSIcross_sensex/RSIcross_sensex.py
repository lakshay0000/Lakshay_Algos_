...
class algoLogic(optOverNightAlgoLogic):
    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        for timeData in df.index: 
            ...
            if timeData-300 in df_5min.index:
                self.strategyLogger.info(
                    f"Datetime: {self.humanTime}\tClose: {df_5min.at[last5MinIndexTimeData[1],'c']}\trsi60: {df_5min.at[last5MinIndexTimeData[1],'rsiCross60']}\trsi50: {df_5min.at[last5MinIndexTimeData[1],'rsiCross50']}\trsi40: {df_5min.at[last5MinIndexTimeData[1],'rsiCross40']}")
            ...
            if not self.openPnl.empty:
                for index, row in self.openPnl.iterrows():
                    ...
                    if row["CurrentPrice"] <= row["Target"]:
                        ...
                        Expiry=row["Expiry"]
                        exitType = "Time Up"
                        self.strategyLogger.info(f"Expiry: {Expiry}\tCurrent Time: {self.timeData}")
            ...
            if ((timeData-300) in df_5min.index):
                ...
                if (callCounter < 3) and (callEntryAllow):
                    if df_5min.at[last5MinIndexTimeData[1], "rsiCross40"] == 1 and df_5min.at[last5MinIndexTimeData[1], "Supertrend"] ==-1:
                        callSym = self.getCallSym(
                            self.timeData, baseSym, df.at[lastIndexTimeData[1], "c"],expiry= Currentexpiry,otmFactor=0,strikeDist=500)
                        ...
                        target = 0.3 * data["c"]
                        stoploss = 1.3 * data["c"]
                        baseSymStoploss = max([df_5min.at[last5MinIndexTimeData[0], "h"], df_5min.at[last5MinIndexTimeData[1], "h"],])
                        ...
                        self.strategyLogger.info(f"expiryEpoch: {expiryEpoch}")
                ...
                if (putCounter < 3) and (putEntryAllow):       
                    if df_5min.at[last5MinIndexTimeData[1], "rsiCross60"] == 1 and df_5min.at[last5MinIndexTimeData[1], "Supertrend"] ==1:
                        ...
                        target = 0.3 * data["c"]
                        stoploss = 1.3 * data["c"]
                        baseSymStoploss = min([df_5min.at[last5MinIndexTimeData[0], "l"], df_5min.at[last5MinIndexTimeData[1], "l"],])
