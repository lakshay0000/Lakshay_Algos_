...
class algoLogic(baseAlgoLogic):
    ...
    def backtestStock(self, stockName, startDate, endDate):
        ...
        df_5min["ema30"] = df_5min["c"].ewm(span=30, adjust=False).mean()
        df_5min["ema3"] = df_5min["c"].ewm(span=3, adjust=False).mean()
        df_5min.dropna(inplace=True)


        df_5min["Bucross"]= np.where((df_5min["ema3"] > df_5min["ema30"]) & (df_5min["ema3"].shift(1) < df_5min["ema30"].shift(1)), 1, 0)
        df_5min["Becross"] = np.where((df_5min["ema3"] < df_5min["ema30"]) & (df_5min["ema3"].shift(1) > df_5min["ema30"].shift(1)), 1, 0)
        ...
        for timeData in df.index:
            ...
            if (stockAlgoLogic.openPnl.empty) & (stockAlgoLogic.humanTime.time() < time(15, 15)):
                if (timeData-300) in df_5min.index:
                    if (df_5min.at[last5MinIndexTimeData[1], "Bucross"] == 1):
                    ...
                    if (df_5min.at[last5MinIndexTimeData[1], "Becross"] == 1):
                        entry_price = df_5min.at[last5MinIndexTimeData[1], "c"]
                        
                        stoploss = df_5min.at[last5MinIndexTimeData[1], "h"]

                        stockAlgoLogic.entryOrder(entry_price, stockName,
                                                  (amountPerTrade//entry_price), "SELL", {"STOPLOSS": stoploss,})
