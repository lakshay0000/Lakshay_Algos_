            ...
            if not self.openPnl.empty:
                if not filtered.empty:
                    if len(filtered) == 2:
                        ...
                        # Trigger on 30% decay/gain in normal mode, 50% in exit_small_mode
                        threshold = 0.50 if exit_small_mode else 0.30
                        triggered = (current_strangle <= strangle_ref_value * (1 - threshold) or
                                     current_strangle >= strangle_ref_value * (1 + threshold))

                        self.strategyLogger.info(f"Strangle: {current_strangle:.2f}, Ref: {strangle_ref_value:.2f}, Threshold: {threshold*100:.0f}%, Triggered: {triggered}")
                        ...
                        if triggered:
                            ...
                            if price1 <= price2:
                                ...
                                if otm is not None:
                                    strangle_ref_value = None  # reset; next iteration captures new strangle ref

                                    if exit_small_mode:
                                        # 50% trigger: exit smaller (row1), enter at bigger price
                                        ...
                                        # 30% trigger: exit bigger (row2), enter at Half_price on bigger's side
                                        if symSide == "CE":
                                            # smaller=CE, bigger=PE → exit PE (row2), enter PE at Half_price
                                            putSym = self.getPutSym(
                                            ...
                                            # smaller=PE, bigger=CE → exit CE (row2), enter CE at Half_price
                                            callSym = self.getCallSym(
                                                self.timeData, baseSym, df.at[lastIndexTimeData[1], "c"], expiry=Currentexpiry, otmFactor=otm)
                            ...
                            # row2 is smaller (price2 < price1)
                                ...
                                Half_price = price2
                                symSide = row2["Symbol"][-2:]

                                if otm is not None:
                                    strangle_ref_value = None  # reset; next iteration captures new strangle ref

                                    if exit_small_mode:
                                        # 50% trigger: exit smaller (row2), enter at bigger price
                                        ...
                                        # 30% trigger: exit bigger (row1), enter at Half_price on bigger's side
                                        if symSide == "CE":
                                            # smaller=CE, bigger=PE → exit PE (row1), enter PE at Half_price
                                            putSym = self.getPutSym(
                                                self.timeData, baseSym, df.at[lastIndexTimeData[1], "c"], expiry=Currentexpiry, otmFactor=otm)
                                            otmstk = re.search(r'(\d+)(?=PE)', putSym).group(1)
                                            ...
                                            # smaller=PE, bigger=CE → exit CE (row1), enter CE at Half_price
                                            callSym = self.getCallSym(
                                                self.timeData, baseSym, df.at[lastIndexTimeData[1], "c"], expiry=Currentexpiry, otmFactor=otm)
                                            otmstk = re.search(r'(\d+)(?=CE)', callSym).group(1)
