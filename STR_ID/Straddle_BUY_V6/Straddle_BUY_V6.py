            ...
            if not self.openPnl.empty:
                if len(self.openPnl) == 2:
                    ...
                    if price1 <= price2 * 0.5:
                        ...
                        Prm = round((StraddlePremium)/50)*50
                        self.strategyLogger.info(f"Straddle Premium at {self.humanTime} is {StraddlePremium}")

                        atmstk = re.search(r'(\d+)(?=CE|PE)', callSym).group(1)

                        if symSide == "CE":
                            
                            callSym, Data_CE = self.OptChain(lastIndexTimeData[1], "CE", df.at[lastIndexTimeData[1], "c"], baseSym, doubled_price, otm=15)
                            
                            symstk= re.search(r'(\d+)(?=CE|PE)', callSym).group(1)

                            if abs(int(symstk) - int(atmstk)) > Prm:
                                self.strategyLogger.info(f"Selected CE strike {symstk}.")
                            ...
                            symstk= re.search(r'(\d+)(?=CE|PE)', putSym).group(1)

                            if abs(int(symstk) - int(atmstk)) > Prm:
                                self.strategyLogger.info(f"Selected PE strike {symstk}.")
