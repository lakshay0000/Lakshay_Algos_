                ...
                if not filtered.empty:
                    if len(filtered) == 2:
                        ...
                        if strangle_ref_value >= 20:
                            ...
                            decay_triggered = current_strangle <= strangle_ref_value * (1 - threshold)
                            gain_triggered = current_strangle >= strangle_ref_value * (1 + gain_threshold)
                            exit_small_on_gain = True
                            self.strategyLogger.info(f"Strangle reference value {strangle_ref_value:.2f} is below 20. Using 50% threshold for decay and gain triggers; gain will exit small leg and enter at big.")
                        ...
                        if decay_triggered or gain_triggered:
                            ...
                            if price1 <= price2:
                                ...
                                if decay_triggered or (gain_triggered and exit_small_on_gain):
                                    # DECAY (or low-ref GAIN): exit smaller (row1), enter at bigger price (price2)
                                    trigger_label = "30% DECAY" if decay_triggered else "GAIN (low ref)"
                                    self.strategyLogger.info(f"{trigger_label} triggered: Exiting smaller {row1['Symbol']} ({price1}), entering at bigger price {price2}.")
                            ...
                            # row2 is smaller (price2 < price1)
                                ...
                                if decay_triggered or (gain_triggered and exit_small_on_gain):
                                    # DECAY (or low-ref GAIN): exit smaller (row2), enter at bigger price (price1)
                                    trigger_label = "30% DECAY" if decay_triggered else "GAIN (low ref)"
                                    self.strategyLogger.info(f"{trigger_label} triggered: Exiting smaller {row2['Symbol']} ({price2}), entering at bigger price {price1}.")
                                    self.exitOrder(row2_index, "Half_Exit")
