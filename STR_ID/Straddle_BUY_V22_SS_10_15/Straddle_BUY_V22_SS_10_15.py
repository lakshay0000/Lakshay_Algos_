...
class algoLogic(optOverNightAlgoLogic):
    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        for timeData in df.index:
            ...
            if ((timeData - 60) in df.index) and self.humanTime.time() >= time(14, 15) and self.humanTime.time() < time(15, 20) and self.humanTime.date() == expiryDatetime.date():
            ...
            if ((timeData - 60) in df.index):
                ...
                # ── BUILD 1 ENTRIES ──
                if b1_sell_empty and self.humanTime.date() == expiryDatetime.date() and self.humanTime.time() >= time(14, 15) and self.humanTime.time() < time(15, 20):
                ...
                # ── BUILD 2 ENTRIES ──
                if b2_sell_empty and self.humanTime.date() == expiryDatetime.date() and self.humanTime.time() >= time(14, 15) and self.humanTime.time() < time(15, 20):
