    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        for timeData in df.index: 
            ...
            if self.humanTime.time() == time(9, 16):
                if Expiry_Day==True and Nxt_day == False:
                    Nxt_day= True
                elif Expiry_Day==True and Nxt_day == True:
                    Take_trade = True
                    Expiry_Day= False
                    Nxt_day = False
            ...
            if ((timeData-60) in df.index):
                ...
                if self.openPnl.empty and self.humanTime.date() != expiryDatetime.date() and self.humanTime.time() > time(15, 20):
                    if self.humanTime.time() == time(15, 25) and Take_trade == True:
                        Take_trade = False
                        if df.at[lastIndexTimeData[1], "c"] < open:
                            ...
                            self.entryOrder(data["c"], callSym, lotSize, "SELL", {"Expiry": expiryEpoch, "Target": target},)
                            ...
                            self.entryOrder(data["c"], putSym, lotSize, "SELL", {"Expiry": expiryEpoch, "Target": target},)
