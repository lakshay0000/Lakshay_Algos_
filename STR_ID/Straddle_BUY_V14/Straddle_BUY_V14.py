    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        for timeData in df.index: 
            ...
            if not self.openPnl.empty and current_ema is not None:
                # Filter only Main_Trade positions
                main_trade_positions = self.openPnl[self.openPnl['Main_Trade'] == True]
                
                if len(main_trade_positions) == 2:     
                    # Get the two positions
                    row1 = main_trade_positions.iloc[0]
                    row2 = main_trade_positions.iloc[1]
                    ...
                    if price1 * 4 <= price2:
                        ...
                        if current_ema < refrence_value:
                            if symSide == "CE":
                                ...
                                if stk > otmstk:
                                    ...
                                    self.openPnl.at[row1_index, "stoploss"] = row["EntryPrice"]
                                    self.openPnl.at[row1_index, "Trailing_Target"] = row["EntryPrice"] * 0.5
                                    self.openPnl.at[row1_index, "Target"] = row["EntryPrice"] * 0.1
                                    self.openPnl.at[row1_index, "Main_Trade"] = False
                                    if Data_CE < 1:
                                        self.squareoff()
                                        if Perc == 2:  
                                            Perc = 1
                                        ...
                                        self.entryOrder(Data_CE, callSym, lotSize, "SELL", {"Expiry": expiryEpoch, "Main_Trade": True},)
                                else:
                                    ...
                                    if Data_PE < 1:
                                        ...
                                        self.entryOrder(Data_PE, putSym, lotSize, "SELL", {"Expiry": expiryEpoch, "Main_Trade": True},)
                        ...
                        if current_ema < refrence_value:
                            ...
                            if symSide == "CE":
                                ...
                                if stk > otmstk:
                                    row2_index = row2.name
                                    self.openPnl.at[row2_index, "stoploss"] = row["EntryPrice"]
                                    self.openPnl.at[row2_index, "Trailing_Target"] = row["EntryPrice"] * 0.5
                                    self.openPnl.at[row2_index, "Target"] = row["EntryPrice"] * 0.1
                                    self.openPnl.at[row2_index, "Main_Trade"] = False
                                    if Data_CE < 1:
                                        self.squareoff()
                                        if Perc == 2:
                                            Perc = 1
            ...
            if not self.openPnl.empty:
                ...
                if callCounter == 3:
                    ce_positions = self.openPnl[self.openPnl['Symbol'].str[-2:] == 'CE'].copy()

                    ce_positions['Pnl'] = pd.to_numeric(ce_positions['Pnl'], errors='coerce')
                    ce_positions = ce_positions.dropna(subset=['Pnl'])

                    if not ce_positions.empty:
                        self.strategyLogger.info(
                            f"CE positions: {ce_positions.to_string()}. Exiting earliest CE position."
                        )

                        earliest_ce_index = ce_positions['Pnl'].idxmax()
                        self.exitOrder(earliest_ce_index, "Earliest_Exit")

                if putCounter == 3:
                    pe_positions = self.openPnl[self.openPnl['Symbol'].str[-2:] == 'PE'].copy()

                    pe_positions['Pnl'] = pd.to_numeric(pe_positions['Pnl'], errors='coerce')
                    pe_positions = pe_positions.dropna(subset=['Pnl'])

                    if not pe_positions.empty:
                        self.strategyLogger.info(
                            f"PE positions: {pe_positions.to_string()}. Exiting earliest PE position."
                        )

                        earliest_pe_index = pe_positions['Pnl'].idxmax()
                        self.exitOrder(earliest_pe_index, "Earliest_Exit")
