...
class algoLogic(optOverNightAlgoLogic):
    ...
    def sl_perc(self, callSym, putSym, callSym_ATM):
        atmstrike = re.search(r'(\d+)(?=CE|PE)', callSym_ATM).group(1)
        CE_strike = re.search(r'(\d+)(?=CE)', callSym).group(1)
        PE_strike = re.search(r'(\d+)(?=PE)', putSym).group(1)

        if abs(int(CE_strike) - int(atmstrike)) > abs(int(PE_strike) - int(atmstrike)):
            dperc = abs(int(CE_strike) - int(atmstrike))/100
            self.strategyLogger.info(f"Selected CE strike {CE_strike} is farther from ATM strike {atmstrike} than PE strike {PE_strike}. Calculated dperc is {dperc}")
        else:
            dperc = abs(int(PE_strike) - int(atmstrike))/100
            self.strategyLogger.info(f"Selected PE strike {PE_strike} is farther from ATM strike {atmstrike} than CE strike {CE_strike}. Calculated dperc is {dperc}")

        if dperc < 2:
            dperc = 2
            self.strategyLogger.info(f"Calculated dperc is less than 2. Setting dperc to minimum value of 2.")
        ...
        for timeData in df.index: 
            ...
            if not self.openPnl.empty and current_ema is not None:
                if len(self.openPnl) == 2:
                    ...
                    if lastIndexTimeData[1] in df.index:
                        callSym_ATM = self.getCallSym(self.timeData, baseSym, df.at[lastIndexTimeData[1], "c"],expiry= Currentexpiry)
                    
                    # Check if one position's price is half or less than the other
                    if (price1 + Dperc * price1) <= price2:
                            ...
                            if symSide == "CE":
                                ...
                                if stk > otmstk:
                                    ...
                                    if Data_CE < 1:
                                        ...
                                        Dperc = self.sl_perc(callSym, putSym, callSym_ATM)
                    ...
                    elif (price2 + Dperc * price2) <= price1:
