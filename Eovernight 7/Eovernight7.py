...
class algoLogic(baseAlgoLogic):
    ...
    def backtest(self, stockName, startDate, endDate):
        ...
        df_5min['ema100'] = df_5min['c'].ewm(span=9, adjust=False).mean()
        df_5min.dropna(inplace=True)

        df_5min["emaUcross"]= np.where((df_5min["c"] > df_5min["ema100"]) & (df_5min["c"].shift(1) < df_5min["ema100"].shift(1)), 1, 0)
        ...
        for timeData in df.index:
            ...
            if ((timeData-300) in df_5min.index) & (stockAlgoLogic.openPnl.empty):
                if (df_5min.at[last5MinIndexTimeData[1], "emaUcross"] == 1) & (df_5min.at[last5MinIndexTimeData[1], "Supertrend"] == 1):
