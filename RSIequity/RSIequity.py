        ...
        for timeData in df.index:
            ...
            if not stockAlgoLogic.openPnl.empty:
                for index, row in stockAlgoLogic.openPnl.iterrows():
                    ...
                    if (df.at[lastIndexTimeData, "rsi"] < 30) and (row["EntryPrice"] <= row["CurrentPrice"]):
                        exitType = "Target"
                        stockAlgoLogic.exitOrder(index, exitType)

                    elif (df.at[lastIndexTimeData, "rsi"] < 30) and (row["EntryPrice"] > row["CurrentPrice"]):
                        Breakeven = True
...
if __name__ == "__main__":
    ...
    portfolio = createPortfolio("/root/Lakshay_Algos/stocksList/nifty50.md", 4)
