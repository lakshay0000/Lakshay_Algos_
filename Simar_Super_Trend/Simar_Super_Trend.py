                ...
                if not self.openPnl.empty:
                    for index, row in self.openPnl.iterrows():
                        ...
                        if self.humanTime.time() >= time(15, 20):
                            ...
                            if (row["PositionStatus"]==1) and (stock not in top5):
                                exitType = "Removed from Top 5"
                                self.exitOrder(index, exitType)


                            elif (row["PositionStatus"]==-1) and (stock not in bottom5):
                                exitType = "Removed from Bottom 5"
                                self.exitOrder(index, exitType)
