    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        for timeData in df.index:
            ...
            if lastIndexTimeData[1] in df.index:
                self.strategyLogger.info(
                    f"Datetime: {self.humanTime}\tClose: {df.at[lastIndexTimeData[1],'c']}\t openpnl: {self.openPnl}")
                print(self.humanTime)
                print(self.openPnl)
            ...
            if (lastIndexTimeData[1] in df.index) & (self.humanTime.time() < time(15, 15)): 
                ...
                if (self.humanTime.time() >= callbOrderTime) & (callO == False):
                    callSym = self.getCallSym(
                        startEpoch, baseSym, df.at[lastIndexTimeData[1], "c"], otmFactor= 3)
                ...
                if ((open_sum + pnnl_sum) < -2000) & (calls == False) & (self.humanTime.time() <= time(14, 30)):
                    callSym = self.getCallSym(
                        startEpoch, baseSym, df.at[lastIndexTimeData[1], "c"])
                ...
                if ((open_sum + pnnl_sum) < -2000) & (puts == False) & (self.humanTime.time() <= time(14, 30)):
                    putSym = self.getPutSym(
                        startEpoch, baseSym, df.at[lastIndexTimeData[1], "c"])
                ...
                if ((open_sum + pnnl_sum) < -2000) & (callb == False) & (self.humanTime.time() <= time(14, 30)):
                    callSym = self.getCallSym(
                        startEpoch, baseSym, df.at[lastIndexTimeData[1], "c"], otmFactor= 3)
                ...
                if ((open_sum + pnnl_sum) < -2000) & (putb == False) & (self.humanTime.time() <= time(14, 30)):
                    putSym = self.getPutSym(
                        startEpoch, baseSym, df.at[lastIndexTimeData[1], "c"], otmFactor= 3)
...
if __name__ == "__main__":
    ...
    endDate = datetime(2021, 4, 30, 15, 30)
