        ...
        for timeData in df.index:
            ...
            if lastIndexTimeData[1] in df.index:
                if self.humanTime.time() == time(9, 30):
                    breakp = df.at[lastIndexTimeData[1], "h"]
                    breakf = df.at[lastIndexTimeData[1], "l"]
            ...
            if (lastIndexTimeData[1] in df.index) & (self.humanTime.time() > time(9, 30)) & (self.humanTime.time() < time(15, 15)) : 
                if (df.at[lastIndexTimeData[1], "c"] < breakf) & (flag == False):
                ...
                if (df.at[lastIndexTimeData[1], "c"] > breakp) & (flag == False):
...
if __name__ == "__main__":
    ...
    startDate = datetime(2023, 3, 1, 9, 15)
