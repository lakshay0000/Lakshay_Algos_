...
class algoLogic(optOverNightAlgoLogic):
    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        for timeData in df.index: 
            ...
            if not self.openPnl.empty:
                if self.humanTime.time() >= time(15, 20) and exit_signal == False:
            ...
            # Stock_Count= tradecount.get('RELIANCE', 0)



            
            # Check for entry signals and execute orders
            if ((timeData-60) in df.index) and (self.humanTime.time() >= time(15, 20)) and exit_signal == False and New_entry:
                if (df.at[lastIndexTimeData[1], "c"] > open_price) and (df.at[lastIndexTimeData[1], "c"] > df_1d.at[prev_day, "h"]):
                    if Trade_count < 2:
                        ...
                        if Trade_count==0:
                            self.entryOrder(entry_price, baseSym, (amountPerTrade//entry_price), "BUY", {"TradeType":"First",})
                            Trade_count += 1
                            consecutive_green_days += 1 
                            New_entry = False
                        else:
                            self.entryOrder(entry_price, baseSym, (amountPerTrade//entry_price), "BUY", {"TradeType":"Second",})
                            Trade_count += 1
                            New_entry = False
                        ...
                        if consecutive_green_days ==2:
                            for index, row in self.openPnl.iterrows():
                                if row.get("TradeType") == "First":
                                    self.exitOrder(index, "Three Green Days Exit")
                                    consecutive_green_days = 0
