    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        for timeData in df.index: 
            ...
            for stock in stock_list:
                ...
                if (self.humanTime.time() == time(9, 20)) and (self.humanTime.time() < time(15, 20)) and New_iteration:
                ...
                if (self.humanTime.time() > time(9, 20)) and (self.humanTime.time() <= time(15, 20)) and New_iteration:
                    ...
                    Perc_top5 = self.get_daily_top_bottom_stocks(gainer_stocks, openEpoch, lastIndexTimeData[1], stock_1min_data, G_L=False)

                    Perc_bottom5 = self.get_daily_top_bottom_stocks(loser_stocks, openEpoch, lastIndexTimeData[1], stock_1min_data, G_L=False)
                    
                    self.strategyLogger.info(f"{self.humanTime} gainer_stocks: {gainer_stocks}, loser_stocks: {loser_stocks}")
                    self.strategyLogger.info(f"{self.humanTime} Perc_gainer: {Perc_top5}, Perc_loser: {Perc_bottom5}")

                    # Sum and count only positive values for gainers
                    Gainer_Sum = sum(Perc_top5)
                    gainer_count = len(Perc_top5)
                    Gainer_Avg = Gainer_Sum / gainer_count if gainer_count > 0 else 0

                    # Sum and count only negative values for losers
                    Loser_Sum = sum(Perc_bottom5)
                    loser_count = len(Perc_bottom5)
                    Loser_Avg = Loser_Sum / loser_count if loser_count > 0 else 0
                    self.strategyLogger.info(f"{self.humanTime} Gainer Sum: {Gainer_Sum}, Loser Sum: {Loser_Sum}, Gainer Avg: {Gainer_Avg}, Loser Avg: {Loser_Avg}")
