...
class algoLogic(optOverNightAlgoLogic):
    ...
    def Straddle_Trade(self, baseSym, df, lastIndexTimeData, expiryEpoch, lotSize, step):
        ...
        self.entryOrder(data["c"], callSym, lotSize, "SELL", {"Expiry": expiryEpoch, "stoploss": stoploss, "Target": target, "Trailing_Target": trailingTarget, "Trade_Type": step},)
        ...
        self.entryOrder(data["c"], putSym, lotSize, "SELL", {"Expiry": expiryEpoch, "stoploss": stoploss, "Target": target, "Trailing_Target": trailingTarget, "Trade_Type": step},)
                ...
                for index, row in self.openPnl.iterrows():
                    ...
                    if self.humanTime.time() >= time(15, 20):
                        ...
                        if row["Trade_Type"] < 3:
                            if row["Trade_Type"] == 1:
                                step = 2
                                self.Straddle_Trade(baseSym, df, lastIndexTimeData, expiryEpoch, lotSize, step)
                            elif row["Trade_Type"] == 2:
                                step = 3
                                self.Straddle_Trade(baseSym, df, lastIndexTimeData, expiryEpoch, lotSize, step)
