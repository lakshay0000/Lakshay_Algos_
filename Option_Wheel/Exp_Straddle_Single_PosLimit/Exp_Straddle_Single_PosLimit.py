...
class algoLogic(optOverNightAlgoLogic):
    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        for timeData in df.index: 
            ...
            # if callCounter > 3:
            #     self.strategyLogger.info(f"{self.humanTime} More than 3 CE positions are open. Current CE count: {callCounter}")
                
            #     # Filter CE positions only
            #     ce_positions = self.openPnl[self.openPnl['Symbol'].str.endswith('CE')]
                
            #     if not ce_positions.empty:
            #         # Find the position with the earliest entry time
            #         earliest_index = ce_positions.sort_values('EntryTime').index[0]
            #         self.exitOrder(earliest_index, "Max Open CE Positions")

            # if putCounter > 3:
            #     self.strategyLogger.info(f"{self.humanTime} More than 3 PE positions are open. Current PE count: {putCounter}")
                
            #     # Filter PE positions only
            #     pe_positions = self.openPnl[self.openPnl['Symbol'].str.endswith('PE')]
                
            #     if not pe_positions.empty:
            #         # Find the position with the earliest entry time
            #         earliest_index = pe_positions.sort_values('EntryTime').index[0]
            #         self.exitOrder(earliest_index, "Max Open PE Positions")
