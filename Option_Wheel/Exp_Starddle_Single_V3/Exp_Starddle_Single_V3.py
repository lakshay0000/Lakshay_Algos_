...
class algoLogic(optOverNightAlgoLogic):
    ...
    def straddle(self, date, IndexPrice, baseSym):     
        ...
        callSymList = []
        putSymList = []
        factorList = []  # Track factors for each pair
        ...
        for i in Factor:
            ...
            factorList.append((i, i_put))  # Store both CE and PE OTM factors
        ...
        if minlist:
            ...
            ce_factor, pe_factor = factorList[minIndex]
            return ce_factor+4, pe_factor+4  # Return the OTM factors for CE and PE
        else:
            return None, None
            ...
            if ((timeData-60) in df.index):
                ...
                if self.humanTime.date() == expiryDatetime.date() and self.humanTime.time() < time(15, 20):
                    ...
                    if StraddleEntryAllowed:
                        ...
                        ce_factor, pe_factor = self.straddle(lastIndexTimeData[1], df.at[lastIndexTimeData[1], "c"], baseSym)
                        
                        # CE Leg
                        callSym = self.getCallSym(
                            self.timeData, baseSym, df.at[lastIndexTimeData[1], "c"],expiry= Currentexpiry, otmFactor=ce_factor)
                        ...
                        putSym = self.getPutSym(
                            self.timeData, baseSym, df.at[lastIndexTimeData[1], "c"],expiry= Currentexpiry, otmFactor=pe_factor)
