    ...
    def straddle(self, date, IndexPrice, baseSym):     
        ...
        for i in Factor:
            ...
            try:
                ...
                self.strategyLogger.info(f"Skipping call option {callSym} for date {datetime.fromtimestamp(date)} due to data fetch error.")
            ...
            try:
                ...
                self.strategyLogger.info(e)
                self.strategyLogger.info(f"Skipping put option {putSym} for date {datetime.fromtimestamp(date)} due to data fetch error.")
        ...
        if minlist:  # Check if minlist is not empty
            ...
            return None, None  # Return None if no valid pairs found
            ...
            if ((timeData-60) in df.index):
                ...
                if self.humanTime.date() == expiryDatetime.date() and self.humanTime.time() < time(15, 20):
                    ...
                    if StraddleEntryAllowed:
                        ...
                        if callSym is None or putSym is None:
                            self.strategyLogger.info(f"{self.humanTime} No valid straddle pairs found. Skipping entry.")
                            continue
