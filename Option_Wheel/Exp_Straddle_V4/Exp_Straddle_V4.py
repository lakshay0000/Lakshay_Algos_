...
class algoLogic(optOverNightAlgoLogic):
    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        # Create a dictionary to store the highest straddle value per day
        straddleData = {}
        ...
        for timeData in df.index: 
            ...
            # Straddle Logic - Identify Straddle Option Pairs and their premiums
            if ((timeData-60) in df.index) and self.humanTime.date() == expiryDatetime.date():
                ...
                try:
                    dataCE = self.fetchAndCacheFnoHistData(callSym, lastIndexTimeData[1])
                    dataPE = self.fetchAndCacheFnoHistData(putSym, lastIndexTimeData[1])
                    self.strategyLogger.info(f"{self.humanTime} Straddle Option Pairs: {callSym} and {putSym}")
                    self.strategyLogger.info(f"{self.humanTime} Straddle Option Premiums: CE: {dataCE['c']} and PE: {dataPE['c']}")
                    StraddleValue = dataCE["c"] + dataPE["c"]
                    self.strategyLogger.info(f"{self.humanTime} Straddle Value: {StraddleValue}")

                    # Store straddle data
                    # Store only the highest straddle value per day
                    currentDate = self.humanTime.date()
                    if currentDate not in straddleData or StraddleValue > straddleData[currentDate]:
                        straddleData[currentDate] = StraddleValue
                except Exception as e:
                    self.strategyLogger.info(e)
                    self.strategyLogger.info(f"{self.humanTime} Error fetching data for {callSym} or {putSym}. Skipping straddle entry for this timestamp.")
                    StraddleValue = None
            ...
            if ((timeData-60) in df.index) and StraddleValue is not None:

                if self.humanTime.date() == expiryDatetime.date() and self.humanTime.time() < time(15, 20):

                    if StraddleEntryAllowed and StraddleValue >= 100 and MaxProfit_Hit == False:  # Example threshold value
                    
                        # CE Leg
                        stoploss = 1.3 * dataCE["c"]
                        target = 0.1 * dataCE["c"]
                        trailingTarget = 0.5 * dataCE["c"]

                        self.entryOrder(dataCE["c"], callSym, lotSize, "SELL", {"Expiry": expiryEpoch, "stoploss": stoploss, "Target": target, "Trailing_Target": trailingTarget, "Trailing_Flag": False},)
                        
                        # PE Leg
                        stoploss = 1.3 * dataPE["c"]
                        target = 0.1 * dataPE["c"]
                        trailingTarget = 0.5 * dataPE["c"]

                        
                        self.entryOrder(dataPE["c"], putSym, lotSize, "SELL", {"Expiry": expiryEpoch, "stoploss": stoploss, "Target": target, "Trailing_Target": trailingTarget, "Trailing_Flag": False},)
        ...
        if straddleData:
            straddleDF = pd.DataFrame(list(straddleData.items()), columns=["Datetime", "StraddleValue"])
            straddleDF.to_csv("StraddleValues.csv", index=False)
            self.strategyLogger.info(f"Straddle values saved to StraddleValues.csv")
