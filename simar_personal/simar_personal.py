...
class algoLogic(optOverNightAlgoLogic):
    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        with open("/root/Lakshay_Algos/stocksList/nifty200.md") as f:
        ...
        for stock in stock_list:
            ...
            if df_1min is None or df_1min.empty:
                self.strategyLogger.info(f"No data for {stock} on {startEpoch}")
                continue
                    ...
                    for index, row in self.openPnl.iterrows():
                        ...
                        if self.humanTime.time() >= time(15, 20):
                            ...
                            if (row["PositionStatus"]==1):
                                if row["CurrentPrice"] >= 1.01*row["EntryPrice"]:
                                    exitType = "Target Hit"
                                    self.exitOrder(index, exitType)
...
if __name__ == "__main__":
    ...
    endDate = datetime(2025, 6, 3, 15, 30)
