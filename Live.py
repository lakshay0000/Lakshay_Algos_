...
from strategyTools.dataLogger import algoLoggerSetup, setup_logger
...
class RSIStrategy:
    ...
    def __init__(self, baseSym):
        ...
        self.fileDir = {
            ...
            "strategyLogs": f"{logFileFolder}/StrategyLog",
            "stockLogs": f"{logFileFolder}/StrategyLog/Symbol",}
        ...
        self.mylogger = SimpleLogger(
            log_dir=f"/root/liveAlgos/algoLogs/{self.algoName}/StrategyLog",
    ...
    def run(self):
        ...
        if self.openPnl.empty and not self.BufferSignal:
            ...
            self.candle_1Min['df'], candle_flag_1Min, self.candle_1Min['last_candle_time'] = OHLCDataFetch(
                ...
                1, 4,  # Fetch 100 candles for buffer
                self.candle_1Min['df'],
                self.logger
            )
            ...
            if df_1Min is not None and not df_1Min.empty:
                ...
                df_1Min['EMA20'] = talib.EMA(df_1Min['c'], timeperiod=20)
                df_1Min['EMA50'] = talib.EMA(df_1Min['c'], timeperiod=50)
                df_1Min['EMA100'] = talib.EMA(df_1Min['c'], timeperiod=100)
                df_1Min['EMA200'] = talib.EMA(df_1Min['c'], timeperiod=200)
                results = taa.stochrsi(df_1Min["c"], length=14, rsi_length=14, k=3, d=3)
                df_1Min["%K"] = results["STOCHRSIk_14_14_3_3"]
                df_1Min["%D"] = results["STOCHRSId_14_14_3_3"]
                df_1Min.dropna(inplace=True)

            # --- Swing High Detection (Breakout) ---
            df_1Min['cross_80'] = (df_1Min["%K"].shift(1) < 80) & (df_1Min["%K"] > 80)
            df_1Min['cross_20'] = (df_1Min["%K"].shift(1) > 20) & (df_1Min["%K"] < 20)
            df_1Min['group_high'] = df_1Min['cross_80'].cumsum()

            first_group_high = df_1Min['group_high'].min()
            df_high = df_1Min[df_1Min['group_high'] > first_group_high].copy()
            ...
            df_1Min['cross_20_down'] = (df_1Min["%K"].shift(1) > 20) & (df_1Min["%K"] < 20)
            df_1Min['cross_80_up'] = (df_1Min["%K"].shift(1) < 80) & (df_1Min["%K"] > 80)
            df_1Min['group_low'] = df_1Min['cross_20_down'].cumsum()

            first_group_low = df_1Min['group_low'].min()
            df_low = df_1Min[df_1Min['group_low'] > first_group_low].copy()
            ...
            df_1Min['ema_max'] = df_1Min[ema_cols].max(axis=1)
            df_1Min['ema_min'] = df_1Min[ema_cols].min(axis=1)
            df_1Min['ema_converged'] = (df_1Min['ema_max'] - df_1Min['ema_min']) < 10
            ...
            if df_1Min['ema_converged'].any():
                last_conv_idx = df_1Min[df_1Min['ema_converged']].index[-1]
                ...
                closes_after_conv = df_1Min.loc[last_conv_idx+1:, 'c']
                ...
                if cross_below_idx is not None and (cross_above_idx is None or cross_below_idx < cross_above_idx):
                    ...
                    closes_after_cross = df_1Min.loc[cross_below_idx+1:, ['c', 'EMA200']]
                    ...
                    closes_after_cross = df_1Min.loc[cross_above_idx+1:, ['c', 'EMA200']]
            ...
            self.save_lists()
            self.save_flags()
            self.BufferSignal = True  # Set BufferSignal to True after initial buffer processing
        ...
        while True:
            ...
            # --- Fetch 1Min Candle Data ---
            self.candle_1Min['df'], candle_flag_1Min, self.candle_1Min['last_candle_time'] = OHLCDataFetch(
                self.symMap[self.baseSym],
                currentDatetime.timestamp(),
                self.candle_1Min['last_candle_time'],
                1, 4, self.candle_1Min['df'], self.logger)
            ...
            df_1Min = self.candle_1Min['df']
            # df_1Min = self.convert_to_ist(df_1Min)
            self.rename_col(df_1Min)
            ...
            second_last_row = df_1Min.iloc[-2]
            last_K = last_row['%K']
            ...
            last_close = df_1Min.iloc[-1]['c']
            second_last_close= df_1Min.iloc[-2]['c']
            last_high = df_1Min.iloc[-1]['h']
            last_low  = df_1Min.iloc[-1]['l']
            # self.logger.info(f"[1Min] => Close: {last_close} | RSI: {last_rsi}")
            ...
            if  len(self.maxlist)>=2 and len(self.minlist)>=2 and (ema_max - ema_min) <10:
            ...
            atm = int(round(last_close / strikeDist) * strikeDist)
            ce_sym = self.getCallSym(currentExpiry, atm, strikeDist)
            pe_sym = self.getPutSym(currentExpiry, atm, strikeDist)


            # --- Check if CE/PE already open ---
            # ce_exists = not self.openPnl[self.openPnl["TradeType"] == "CE"].empty
            # pe_exists = not self.openPnl[self.openPnl["TradeType"] == "PE"].empty
            ...
            if not self.openPnl.empty:
                ...
                for idx, row in self.openPnl.iterrows():
                    ...
                    if symSide == "PE":
                    # Target exit
                        if curr_price < 0.90 * entry_price:
                        ...
                        # Stoploss Exit
                        elif (last_close <= (Index_price -10)):
                            hedgeIndex = self.getHedgeIndex(idx)
                        ...
                        elif (last_close < last_ema200) and (second_last_close > second_Last_ema200):
                            hedgeIndex = self.getHedgeIndex(idx)
            ...
            #     self.mylogger.log(f"TEST_ENTRY: SELL PE {pe_sym} @ {pe_price }")
    ...
    def save_open_positions(self):
        """Save open positions to JSON and CSV."""
        open_dir = f"/root/liveAlgos/algoLogs/{self.algoName}/OpenPositions"
        os.makedirs(open_dir, exist_ok=True)
        # Save as CSV
        self.openPnl.to_csv(os.path.join(open_dir, "open_positions.csv"), index=False)
        # Save as JSON
        self.openPnl.to_json(os.path.join(open_dir, "open_positions.json"), orient="records", date_format="iso")

    def load_open_positions(self):
        """Load open positions from JSON if available."""
        open_path = f"/root/liveAlgos/algoLogs/{self.algoName}/OpenPositions/open_positions.json"
    ...
    def save_closed_positions(self):
        """Append closed positions to JSON and CSV, keeping all records."""
        closed_dir = f"/root/liveAlgos/algoLogs/{self.algoName}/ClosedPositions"
        os.makedirs(closed_dir, exist_ok=True)
        csv_path = os.path.join(closed_dir, "closed_positions.csv")
        json_path = os.path.join(closed_dir, "closed_positions.json")

        # Load existing closed positions if they exist
        if os.path.exists(csv_path):
            existing_df = pd.read_csv(csv_path)
            # Concatenate and drop duplicates based on unique keys (e.g., 'Key' and 'ExitTime')
            combined_df = pd.concat([existing_df, self.closedPnl], ignore_index=True)
            combined_df.drop_duplicates(subset=["Key", "ExitTime"], inplace=True)
        else:
            combined_df = self.closedPnl.copy()
    ...
    def load_closed_positions(self):
        """Load closed positions from JSON if available."""
        closed_path = f"/root/liveAlgos/algoLogs/{self.algoName}/ClosedPositions/closed_positions.json"
        if os.path.exists(closed_path):
            self.closedPnl = pd.read_json(closed_path)
        else:
            self.closedPnl = pd.DataFrame()


    def save_lists(self):
        lists_path = f"/root/liveAlgos/algoLogs/{self.algoName}/lists.json"
        lists = {
    ...
    def save_flags(self):
        """Save strategy flags to JSON."""
        flags_path = f"/root/liveAlgos/algoLogs/{self.algoName}/flags.json"
        flags = {
    ...
    def load_flags(self):
        ...
        if os.path.exists(flags_path):
            with open(flags_path, "r") as f:
                ...
                self.BufferEntryCall= flags.get("BufferEntryCally", False)
