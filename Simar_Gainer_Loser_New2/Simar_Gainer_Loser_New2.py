        ...
        for timeData in df.index: 
            for stock in stock_list:
                ...
                if state["i"]== 60:
                    ...
                    state["high_list"] = []
                    state["low_list"] = []
                    state["m_upper"] = (state["Today_high"] - state["prev_DH"]) / (375+state["j"]+high_index)
                    state["m_lower"] = (state["Today_low"]  - state["prev_DL"]) / (375+state["j"]+low_index)
                    state["j"] = state["j"] + 60

                    self.strategyLogger.info(f"{self.humanTime} 1 Hour Completed. High List: {state['Today_high']}, Low List: {state['Today_low']}")
                ...
                if ((timeData-60) in df_1min.index) and (self.humanTime.time() < time(15, 20)):
                    ...
                    if (stock in selected_stocks) and (state["stockcount"] ==0):
