    ...
    def run(self, startDate, endDate, baseSym, indexSym):
        ...
        for timeData in df.index: 
            ...
            if m_upper is None and m_lower is None:
                m_upper = (Today_high - prev_DH) / (375+k)
                m_lower = (Today_low - prev_DL) / (375+k)
            ...
            if not self.openPnl.empty:
                for index, row in self.openPnl.iterrows():
                    ...
                    elif row["TradeType"] == "NonMid":

                        if (row["PositionStatus"]==1) and df.at[lastIndexTimeData[1], "c"] < upper_ray:
                    ...
                    elif row["TradeType"] == "Mid":

                        if (row["PositionStatus"]==1) and df.at[lastIndexTimeData[1], "c"] < Today_open:
                            exitType = "Upper Ray Hit"
                            self.exitOrder(index, exitType)
                            MidDay=True


                        elif (row["PositionStatus"]==-1) and df.at[lastIndexTimeData[1], "c"] > Today_open:
                            exitType = "Lower Ray Hit"
                            self.exitOrder(index, exitType)
                            MidDay=True
            ...
            if ((timeData-60) in df.index) and self.openPnl.empty and (self.humanTime.time() == time(9, 18)):
                if (upper_ray > Today_open and lower_ray > Today_open) or (upper_ray < Today_open and lower_ray < Today_open):
                    if df.at[lastIndexTimeData[1], "c"] > upper_ray:
                        self.strategyLogger.info(f"Placing NonMid BUY Order at {df.at[lastIndexTimeData[1], 'c']}")

                        entry_price = df.at[lastIndexTimeData[1], "c"]

                        self.entryOrder(entry_price, baseSym, (amountPerTrade//entry_price), "BUY", {"TradeType": "NonMid"})
                        
                    if df.at[lastIndexTimeData[1], "c"] < lower_ray:
                        self.strategyLogger.info(f"Placing NonMid SELL Order at {df.at[lastIndexTimeData[1], 'c']}")

                        entry_price = df.at[lastIndexTimeData[1], "c"]


                        self.entryOrder(entry_price, baseSym, (amountPerTrade//entry_price), "SELL", {"TradeType": "NonMid"})

                else:
                    if df.at[lastIndexTimeData[1], "c"] > upper_ray:
                        self.strategyLogger.info(f"Placing Mid BUY Order at {df.at[lastIndexTimeData[1], 'c']}")

                        entry_price = df.at[lastIndexTimeData[1], "c"]

                        self.entryOrder(entry_price, baseSym, (amountPerTrade//entry_price), "BUY", {"TradeType": "Mid"})
                        
                    if df.at[lastIndexTimeData[1], "c"] < lower_ray:
                        self.strategyLogger.info(f"Placing Mid SELL Order at {df.at[lastIndexTimeData[1], 'c']}")

                        entry_price = df.at[lastIndexTimeData[1], "c"]


                        self.entryOrder(entry_price, baseSym, (amountPerTrade//entry_price), "SELL", {"TradeType": "Mid"})


            if (self.humanTime.time() > time(9, 18)) and (self.humanTime.time() < time(15, 20)):

                if ((timeData-60) in df.index) and self.openPnl.empty and MidDay:
                    if df.at[lastIndexTimeData[1], "c"] > upper_ray:
                        self.strategyLogger.info(f"Placing Mid BUY Order at {df.at[lastIndexTimeData[1], 'c']}")
